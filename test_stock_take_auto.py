...
assert cancelled_in_duty[0]['cancelled_at'] != '', '撤销时间不应为空'
print(f'  ✅ 值班员列表能查到已撤销: {cancelled_in_duty[0]["stock_take_no"]}')
print(f'    列表直接显示: 撤销人={cancelled_in_duty[0]["canceller_name"]}, 撤销时间={cancelled_in_duty[0]["cancelled_at"]}')

# 验证其他状态的撤销字段为空
all_st = requests.get(f'{BASE_URL}/stock-takes', headers=get_headers(admin_token)).json()
for st in all_st:
    if st['status'] != 'cancelled':
        assert st['canceller_name'] == '', f'非撤销记录{cancelled_in_duty[0]["stock_take_no"]}的canceller_name应空'
        assert st['cancelled_at'] == '', f'非撤销记录{cancelled_in_duty[0]["stock_take_no"]}的cancelled_at应空'
print(f'  ✅ 非撤销记录的撤销字段正确为空')
